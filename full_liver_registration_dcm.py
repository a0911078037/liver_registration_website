...
before_niigz_path='img1/'
after_niigz_path='img2/'

before_dicom=glob.glob(before_niigz_path+'*')
before_dicom.sort(key=rf.natural_keys)
before=[]
for i in before_dicom:
    dicom=sitk.ReadImage(i)
    img_array = sitk.GetArrayFromImage(dicom)
    before.append(img_array)
before=np.array(before).squeeze()

after_dicom=glob.glob(after_niigz_path+'*')
after_dicom.sort(key=rf.natural_keys)
after=[]
for i in after_dicom:
    dicom=sitk.ReadImage(i)
    img_array = sitk.GetArrayFromImage(dicom)
    after.append(img_array)
after=np.array(after).squeeze()

path_list=[before_niigz_path,after_niigz_path]
dcm_list=[before,after]


slice_thickness_before=float(sitk.ReadImage(before_dicom[0]).GetMetaData("0018|0050"))
slice_thickness_after=float(sitk.ReadImage(after_dicom[0]).GetMetaData("0018|0050"))
